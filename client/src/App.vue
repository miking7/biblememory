  ...
  <div class="container mx-auto px-4 py-8 max-w-5xl">
    ...
    <div class="glass-card rounded-2xl shadow-2xl overflow-hidden fade-in">
      ...
      <div v-show="currentTab === 'list'" class="p-8">
        ...
        <div class="space-y-4">
          <template v-for="verse in filteredVerses" :key="verse.id">
                  ...
                  <template v-if="verse.tags && verse.tags.length > 0">
                    <div class="flex flex-wrap items-center gap-1">
                      <template v-for="tag in verse.tags" :key="tag.key">
                        <span class="px-2 py-1 bg-purple-50 text-purple-700 rounded text-xs font-medium" v-text="formatTagForDisplay(tag)"></span>
                      </template>
                    </div>
                  </template>
                </div>
