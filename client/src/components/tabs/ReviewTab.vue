      ...
      <template v-if="currentReviewVerse">
        ...
        <div class="relative">
          ...
          <div ref="cardElement"
               ...
               :style="{
                 transform: `translateX(${isSwiping ? swipeOffset : transitions.cardOffset.value}px)`,
                 transition: isSwiping ? 'none' : `transform ${transitions.transitionDuration.value} ease-out, opacity ${transitions.transitionDuration.value} ease-out`,
                 opacity: transitions.cardVisible.value ? 1 : 0,
                 touchAction: 'pan-y'
               }"
...
const props = defineProps<{
  ...
  // Lets this component register its card transitions with the review
  // orchestrator so every navigation source animates consistently.
...
// Card element ref for swipe detection and animations
const cardElement = ref<HTMLElement | null>(null) as Ref<HTMLElement | null>

// Initialize card transitions (component owns its DOM and animations)
const transitions = useCardTransitions(cardElement)

// Register our transitions with the review orchestrator so navigate()
// (and therefore arrows, swipe, keyboard, Got it/Again and card clicks)
// all drive the same exit/entry animation.
...
const { isSwiping, swipeOffset } = useSwipeDetection(cardElement, {
  onSwipeLeft: () => emit('navigate', { direction: 'next' }),
  onSwipeRight: () => emit('navigate', { direction: 'previous' }),
  threshold: 50,
